"""
train_model_a.py  —  Real-world Plant Model (archive_clean dataset)
====================================================================
• Input:     archive_clean/   (merged real smartphone photos, ~20 classes)
• Model:     EfficientNetV2S  (21M params, strong on real photos)
• Expected:  85-92% accuracy
• Output:    ml/output/model_a.tflite  +  ml/output/label_map_a.json
...
# ── Config ────────────────────────────────────────────────────────
DATA_DIR    = Path(r"c:\Users\PAAL-DABBA\Desktop\backup-pdd\archive_clean")
...
LABEL_MAP_A = OUTPUT_DIR / "label_map_a.json"

IMG_SIZE    = 224
BATCH_SIZE  = 32    # safe for 6GB VRAM
P1_EPOCHS   = 15   # head-only training
P2_EPOCHS   = 20   # fine-tuning
...
# Augmentation for real-world photos
...
def build_model():
    ...
    outputs = keras.layers.Dense(num_classes, dtype="float32")(x)  # float32 for stability
    return keras.Model(inputs, outputs)

def unfreeze_conv_only(model):
    """Unfreeze Conv layers only — keep BatchNorm in inference mode."""
        ...
        if hasattr(layer, "layers"):  # is a sub-model
            for l in layer.layers:
                if isinstance(l, keras.layers.Conv2D):
                    ...
                    l.trainable = False
    model.layers[1].trainable = True  # base model
...
class SaveState(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        last_w = CKPT_DIR / "last_epoch.weights.h5"
        self.model.save_weights(str(last_w))
...
if state["phase"] == 1:
    ...
    model.fit(
        ...
        epochs=P1_EPOCHS, initial_epoch=init_epoch,
        callbacks=[
            ...
            SaveState(phase=2),   # save with phase=2 so resume skips here
        ],
    )
...
if state["phase"] == 2:
    ...
    model.fit(
        train_ds, validation_data=val_ds,
        epochs=P2_EPOCHS, initial_epoch=init_epoch,
        callbacks=[
            keras.callbacks.ModelCheckpoint(str(BEST_CKPT), save_best_only=True,
                monitor="val_accuracy", save_weights_only=True),
...
print(f"\n✅ Model A saved: {TFLITE_OUT}  ({size_mb:.1f} MB)")
print(f"✅ Label map  A: {LABEL_MAP_A}  ({num_classes} classes)")
print(f"\nStart server with: python ml/server.py")
